...
const CashFlowChart: React.FC<{ transactions: Transaction[], period: Period }> = ({ transactions, period }) => {
    const dataPoints = useMemo(() => {
        const formatters = {
            month: new Intl.DateTimeFormat('pt-BR', { day: 'numeric' }),
            quarter: new Intl.DateTimeFormat('pt-BR', { month: 'short' }),
            year: new Intl.DateTimeFormat('pt-BR', { month: 'short' }),
            all: new Intl.DateTimeFormat('pt-BR', { month: 'short', year: '2-digit' }),
        };
        ...
        transactions.forEach(t => {
            ...
            let key = '';
            if (period === 'month') key = `Dia ${formatters.month.format(date)}`;
            else if (period === 'quarter') key = formatters.quarter.format(date);
            else if (period === 'year') key = formatters.year.format(date).replace('.', '');
            else key = formatters.all.format(date);
            ...
            {dataPoints.map((dp, i) => (
                <div key={i} className="flex-1 flex flex-col items-center group relative">
                    <div className="w-full h-full flex items-end gap-1 justify-center">
                       ...
                       <div className="w-1/3 bg-blue-500/50 rounded-t-md transition-all duration-300 group-hover:bg-blue-500" style={{ height: `${(dp.profit / maxValue) * 100}%` }}></div>
                    </div>
                    <span className="text-xs mt-1 font-semibold">{dp.label}</span>
